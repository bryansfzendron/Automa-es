...
caminho = (r'C:\Gdrive\Dados_Publicos_SP\Saude\COVID\Dados Covid Municipios SP.csv')
dataArquivo = str(datahj.strftime("%d-%m-%Y"))
nmArquivo = str(datahj.strftime("Dados Covid Municipios SP.py - %d_%m_%Y.txt"))
DataEmail = str(datahj.strftime("%d/%m/%Y - %H:%M:%S"))
url = f'https://www.saopaulo.sp.gov.br/wp-content/uploads/{anoO}/{mesO}/{dataontem}_dados_covid_municipios_sp.csv'
...
def enviaEmail():
    ...
    enviarEmail['subject'] = "Erro na Execução Dados Covid Municipios SP.py na data "+DataEmail
