...
caminho = (r'C:/Gdrive/Dados_Publicos_SP/Saude/Vacinacao/EvolucaoAplicacaoDoses.csv')
dataArquivo = str(datahj.strftime("%d-%m-%Y"))
nmArquivo = str(datahj.strftime("EvolucaoAplicacaoDoses.py - %d_%m_%Y.txt"))
DataEmail = str(datahj.strftime("%d/%m/%Y - %H:%M:%S"))
url = f'https://www.saopaulo.sp.gov.br/wp-content/uploads/{anoO}/{mesO}/{dataontem}_evolucao_aplicacao_doses.csv'
...
def enviaEmail():
    ...
    enviarEmail['subject'] = "Erro na Execução EvolucaoAplicacaoDoses.py na data "+DataEmail
